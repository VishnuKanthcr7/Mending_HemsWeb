...
if (isset($_POST['add'])) {
    ...
    $_POST['publish'] = ($_POST['publish'] ?? null) === 'on' ? 1 : 0;
    $_POST['accept_fabric'] = ($_POST['accept_fabric'] ?? null) === 'on' ? 1 : 0;
    ...
    if (!$errors) {
        $query = "INSERT INTO products (price, images, publisher, sizes, attributes, active, name, category, description, accept_fabrics) VALUES ({$_POST['price']}, '{$_POST['images']}', {$_SESSION['uid']}, '{$_POST['size']}', '{$_POST['attributes']}', {$_POST['publish']}, '{$_POST['name']}', '{$_POST['category']}', '{$_POST['description']}', {$_POST['accept_fabric']})";
        $db->query($query);
        // insert customization data
        if (isset($_POST['customize']) && isValidJSON($_POST['customize'])) {
            $pid = $db->query("SELECT id FROM products WHERE name = '{$_POST['name']}' ORDER BY id DESC LIMIT 1")->fetch_assoc()['id'] ?? null;
...
<body class="grid grid-cols-12 font-roboto overflow-hidden">
    ...
    <div class="col-span-7 divide-y  h-screen overflow-y-auto">
        ...
        ?>
            <p class="text-green-500 p-3">Product was added</p>
        <?php
        }
        ?>
        <form action="#" method="post" class="p-3" enctype="multipart/form-data">
            <label for="name">
                <input type="text" name="name" class="rounded-md border border-stone-300 shadow-sm p-3 w-full mt-3" placeholder="Name" required />
            </label>
            <label for="price">
                <input type="number" min="1" name="price" class="rounded-md border border-stone-300 shadow-sm p-3 w-full mt-3" placeholder="Price" required />
            </label>
            <div class="grid grid-cols-12 gap-2">
                <div class="col-span-6">
                    <label for="size">
                        <input type="text" name="size" class="rounded-md border border-stone-300 shadow-sm p-3 w-full mt-3" placeholder="Sizes comma seperated SM,MD,XL" value="SM" required />
                    </label>
                </div>
                <div class="col-span-6">
                    <label for="category">
                        <input type="text" name="category" class="rounded-md border border-stone-300 shadow-sm p-3 w-full mt-3" placeholder="Category" required />
                    </label>
                </div>
            </div>
            <div class="files">
                <label for="file">
                    <input type="file" accept="image/png, image/jpeg" multiple name="file[]" class="rounded-md border border-stone-300 shadow-sm p-3 w-full mt-3" required />
                </label>
            ...
            <div class="grid grid-cols-12">
                <div class="col-span-6 mt-3 flex items-center justify-center">
                    <p>Accept fabrics</p>
                    <label for="name">
                        <input type="checkbox" name="accept_fabric" class="m-1"  />
                    </label>
                </div>
                <div class="col-span-6 mt-3 flex items-center justify-center">
                    <p>Publish</p>
            ...
            <label for="description">
                <textarea name="description" placeholder="Description" class="rounded-md border border-stone-300 shadow-sm p-3 w-full mt-3" cols="30" rows="8" required></textarea>
            </label>
            <label for="attributes">
                <textarea name="attributes" placeholder="Attributes (json)" class="rounded-md border <?php echo isset($errors['attributes']) ? 'border-rose-500' : 'border-stone-300' ?> shadow-sm p-3 w-full mt-3" cols="30" rows="8" required></textarea>
            </label>
            <p class="mt-1">Customization (optional)</p>
            <label for="customize">
                <textarea name="customize" placeholder="Customize (json)" class="rounded-md border <?php echo isset($errors['customize']) ? 'border-rose-500' : 'border-stone-300' ?> shadow-sm p-3 w-full mt-3" cols="30" rows="8"></textarea>
            </label>
            <input type="submit" name="add" value="Add Product" class="w-full bg-stone-800 text-white p-3 rounded-md mt-3">
        </form>
    </div>
